...
API REST de banco digital simplificado desenvolvida em Java 17 com Spring Boot, seguindo Arquitetura Hexagonal (Ports & Adapters).
...
Todos os 19 testes passam (unitários + integração com H2).

---

## Arquitetura

O projeto segue **Arquitetura Hexagonal (Ports & Adapters)**, garantindo total isolamento do domínio em relação ao framework.

```
src/main/java/com/ivan/amado/bank/
├── domain/                        ← Núcleo de negócio (sem Spring)
│   ├── model/Account.java         ← Entidade de domínio
...
│   │   └── output/                ← Interfaces de saída (repos, notifier)
│   ├── usecase/                   ← Implementações dos casos de uso
│   └── dto/                       ← DTOs de request/response
└── infrastructure/                ← Adapters e framework
    ├── controller/                ← REST Controllers
    ├── persistence/               ← JPA entities, repos, adapters
    ├── notification/              ← Adapter de notificação
    └── exception/                 ← GlobalExceptionHandler
```

### Regras principais
- O **domínio** não depende do Spring nem de infraestrutura
- Os **casos de uso** orquestram a lógica sem conhecer detalhes de persistência
- A **infraestrutura** implementa as interfaces definidas pelo domínio/aplicação
...
- A anotação `@Transactional` em `TransferUseCaseImpl.transfer()` garante atomicidade: débito + crédito + registro ocorrem em uma única transação
...
|---|---|
| Arquitetura Hexagonal | Isolamento total do domínio, facilita testes e substituição de adapters |
| H2 in-memory | Simplicidade para o contexto do teste técnico; substituível por PostgreSQL via adapter |
